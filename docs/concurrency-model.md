...
| Whisper timeout                                             | `transcription.ts` (`new OpenAI({ timeout, maxRetries })`)           | `TRANSCRIPTION_TIMEOUT_MS` = 20s                                         | a slow/stalled transcription can't pin the voice handler for minutes                                                                  |
| Inbound handler backstop (`createHandlerTimeoutMiddleware`) | first `bot.use()` in `telegram.ts`                                   | `TELEGRAM_HANDLER_TIMEOUT_MS` = 60s                                      | grammy's poll loop — no single update can block it longer than the budget; on timeout the handler is abandoned and the loop continues |
